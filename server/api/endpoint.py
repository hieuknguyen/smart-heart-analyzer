...
from server.services.predict_service import save_heart_disease_result, save_ecg_result, save_chat_history
from server.predict.chatbot_predictor import ChatRequest, generate_response
from fastapi import APIRouter, Depends, Form
from server.predict.heart_disease_predictor import predict_heart_disease
...
async def predict_ecg_api(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    result = ecg_predictor.predict(file_path) 

    save_ecg_result(db, file.filename, result)
    return {"type": "ecg", "prediction": result}

@router.post("/chatbot")
async def chatbot_api(chat_request: ChatRequest, db: Session = Depends(get_db)):
